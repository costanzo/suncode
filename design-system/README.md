...
The React routes are the visual contract for review. The upper-right navigation switches between the four primary layers—Core, Components, Platforms, and Projects—and intentionally shows only their names. Each switch opens a responsive card-based module index; the cards omit redundant paths and action labels, while detail pages retain ownership metadata. Platforms includes Desktop, Mobile, TUI, and Web; only Desktop is implemented for Phase 1, while Mobile, TUI, and Web remain explicitly deferred. The left sidebar exposes the active layer as an expandable tree, and every expanded child entry across all four layers is an independently addressable route rather than an in-page scroll action. Parent pages remain card indexes so each child inventory can grow independently. All React source is organized under `src/`; each component folder owns its implementation and `index.js` export, while `src/components/universal/modules/` owns category pages that compose those specimens. Token sources live under `src/styles/tokens/`; `src/styles/review.css` and `src/styles/browser.css` carry shared review primitives and the catalog shell. Avalonia runtime resources in `apps/desktop-avalonia/App.axaml` must keep the same semantic names and values.
