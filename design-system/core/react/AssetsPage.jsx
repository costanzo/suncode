    ...
    <>
      ...
      <Section title="Brand marks">
        <div className="logo-specimens">
          <div className="logo-on-light"><img src="./core/assets/logos/suncode-logo.svg" alt="SunCode full logo" /><code>suncode-logo.svg</code></div>
          <div className="logo-on-dark"><img src="./core/assets/logos/suncode-logo-small.svg" alt="SunCode compact logo" /><code>suncode-logo-small.svg</code></div>
        </div>
      </Section>
      <Section title="Interface icons" description="SVG symbols use one restrained visual family; accessible names belong to their controls, not the asset file.">
        <div className="asset-icons">
          {icons.map((name) => <div key={name}><img src={`./core/assets/icons/${name}.svg`} alt="" /><code>{name}</code></div>)}
        </div>
      </Section>
      <Section title="Fonts" description="The review browser uses installed fallbacks; runtime packaging must license and include a font before depending on it.">
