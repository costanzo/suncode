...
The navigation also exposes the six immutable built-in specialists: Architect, UI/UX Agent, Product Agent, Software Engineering Agent, QA Agent, and SRE Agent. Their pages are generated from one shared review catalog so the top-level Agents navigation, module index, and Desktop Settings specimen use the same names, IDs, descriptions, allowlists, and boundaries.
