...
const settingsGuide = { tabs: {
  actions: ["Choose Defaults, Appearance, Network, or Logging from the left navigation.", "Use the chevron beside Model providers to collapse or expand its provider links.", "Select a provider to edit its OpenAI-compatible URL or credential.", "Use Reset default to restore a provider's built-in URL.", "Use the folder buttons in Logging to choose log and image storage directories.", "Edit a control and use its save action; use Done to return to ProjectHub."],
  ...
  logic: ["Settings are local to the embedded agent and are grouped by defaults, appearance, network, logging, and providers.", "Provider URL changes and default resets are persisted and applied to subsequent requests without changing credentials or models.", "Provider credentials are masked; only the first and last four characters are shown for recognition.", "Saving updates local configuration state and does not grant new machine authority."],
} };
...
function SettingsNav({ page, setPage, providersExpanded, setProvidersExpanded }) {
  return <aside className="settings-nav"><nav aria-label="Settings sections">
    <div className="settings-nav-list">{navItems.map((item) => <button key={item.id} type="button" aria-current={page === item.id ? "page" : undefined} className={`settings-nav-item ${page === item.id ? "is-selected" : ""}`} onClick={() => setPage(item.id)}><Icon name={item.icon} size={15} /><span>{item.label}</span></button>)}<div className="settings-nav-models"><button type="button" className={`settings-nav-item settings-nav-parent ${page === "providers" ? "is-selected" : ""}`} aria-current={page === "providers" ? "page" : undefined} aria-label={`${providersExpanded ? "Collapse" : "Expand"} model providers`} aria-expanded={providersExpanded} aria-controls="settings-provider-list" onClick={() => { setPage("providers"); setProvidersExpanded((expanded) => !expanded); }}><Icon name="components" size={15} /><span>Model providers</span><Icon name="chevron-right" size={13} className={`settings-nav-chevron ${providersExpanded ? "is-rotated" : ""}`} /></button>{providersExpanded && <div className="settings-provider-list" id="settings-provider-list">{Object.entries(providerCatalog).map(([id, provider]) => <button key={id} type="button" aria-current={page === `provider:${id}` ? "page" : undefined} className={`settings-nav-item settings-nav-provider ${page === `provider:${id}` ? "is-selected" : ""}`} onClick={() => setPage(`provider:${id}`)}><span>{provider.label}</span></button>)}</div>}</div></div>
  </nav></aside>;
}
...
function ProviderPanel({ providerId, onSave, endpoint, onEndpointChange }) {
  ...
  const saveEndpoint = () => { const normalized = draftEndpoint.trim().replace(/\/+$/, ""); onEndpointChange(normalized); setDraftEndpoint(normalized); onSave(); };
  const resetEndpoint = () => { setDraftEndpoint(provider.endpoint); onEndpointChange(provider.endpoint); onSave(); };
  const endpointIsDefault = endpoint === provider.endpoint && draftEndpoint === provider.endpoint;
  return <div className="settings-panel-content"><div className="settings-panel-heading"><h2>{provider.label}</h2><p>Configure the provider URL and credential used by the local agent.</p></div><div className="settings-panel-section"><span className="settings-section-label">Provider URL</span><SettingRow label="API base URL" hint="OpenAI-compatible URL used for subsequent requests."><input className="field settings-key mono" type="url" value={draftEndpoint} onChange={(event) => setDraftEndpoint(event.target.value)} aria-label={`${provider.label} provider URL`} spellCheck="false" /></SettingRow><div className="settings-actions"><Button variant="primary" size="sm" disabled={!draftEndpoint.trim() || draftEndpoint.trim() === endpoint} onClick={saveEndpoint}>Save URL</Button><Button size="sm" disabled={endpointIsDefault} onClick={resetEndpoint}>Reset default</Button></div></div><div className="settings-divider" /><div className="settings-panel-section"><span className="settings-section-label">Credential</span><div className={`settings-credential-status ${configured ? "is-configured" : ""}`}><span className="settings-status-dot" /><div>{configured ? <><strong>API key configured</strong><code aria-label={`API key starts with ${maskedKey.slice(0, 4)} and ends with ${maskedKey.slice(-4)}`}>{maskedKey}</code></> : <><strong>No API key configured</strong><small>Add a key to enable this provider.</small></>}</div></div><input className="field settings-key mono" type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={configured ? "Paste a new key to replace it" : provider.placeholder} aria-label={`${provider.label} API key`} autoComplete="new-password" /><div className="settings-actions"><Button variant="primary" size="sm" disabled={!apiKey.trim()} onClick={saveKey}>{configured ? "Replace key" : "Save key"}</Button><Button variant="danger" size="sm" onClick={removeKey} disabled={!configured}>Remove key</Button></div></div><div className="settings-divider" /><div className="settings-panel-section"><span className="settings-section-label">Available models</span><div className="settings-model-list">{provider.models.map((model) => <code key={model}>{model}</code>)}</div></div></div>;
}

export function SettingsPage() {
  const [page, setPage] = useState("defaults");
  const [providersExpanded, setProvidersExpanded] = useState(true);
  const [providerEndpoints, setProviderEndpoints] = useState(() => Object.fromEntries(Object.entries(providerCatalog).map(([id, provider]) => [id, provider.endpoint])));
  ...
  const renderPanel = () => {
    ...
    if (page === "providers") return <ProvidersPanel endpoints={providerEndpoints} onSelect={(providerId) => { setPage(`provider:${providerId}`); setStatus(""); }} />;
    if (page.startsWith("provider:")) { const providerId = page.split(":")[1]; return <ProviderPanel key={page} providerId={providerId} endpoint={providerEndpoints[providerId]} onEndpointChange={(endpoint) => setProviderEndpoints((current) => ({ ...current, [providerId]: endpoint }))} onSave={save} />; }
    return <DefaultsPanel onSave={save} />;
  };
  return <><PageHeader title="Settings" description="The Avalonia desktop settings window for local defaults, security, diagnostics, and provider credentials." path="projects/desktop/settings/" /><WindowSizeNote width="900" height="672" minimumWidth="720" minimumHeight="552" /><Section id="settings-window" title="Desktop settings" description="A focused settings window whose outer title bar and controls are native to the operating system."><WorkspaceGuideState className="settings-guide-state" title="Settings controls" description="Navigate local defaults, security, diagnostics, and provider credentials." guide={settingsGuide} side="right" open={guideOpen} onToggle={() => setGuideOpen((open) => !open)} onClose={() => setGuideOpen(false)}><NativeWindowFrame platform="macos" title="Settings" className="settings-window"><div className="settings-toolbar"><strong>Settings</strong><Button variant="primary" size="sm" onClick={navigateBack}>Done</Button></div><div className="settings-body"><SettingsNav page={page} setPage={(nextPage) => { setPage(nextPage); setStatus(""); }} providersExpanded={providersExpanded} setProvidersExpanded={setProvidersExpanded} /><main className="settings-panel">{renderPanel()}{status && <div className="settings-global-status" role="status">{status}</div>}</main></div></NativeWindowFrame></WorkspaceGuideState></Section></>;
}
