  ...
  withDependencies: {
    tabs: {
      ...
      style: [
        ...
        "Root path subtitles use 9px monospace text and muted contrast.",
        "Folders, Markdown, code, and configuration files receive different 14px icons.",
      ],
      logic: [
        "Dependencies extend read authority but do not grant write, process, or Git access.",
        "Each dependency is rendered as a named folder with its registered path.",
        "The tree keeps project and dependency roots at the same hierarchy level.",
      ],
    },
  },
  constrained: {
    tabs: {
      ...
      logic: [
        ...
        "Horizontal scrolling preserves the canonical path instead of hiding segments.",
        "Tree depth remains data-driven so additional nesting does not change row geometry.",
      ],
    },
  },
};
...
export function WorkspaceExplorerPage() {
  ...
  const states = [
    {
      id: "withoutDependencies",
      title: "Without dependencies",
      description: "The project tree is available without registered dependency roots.",
      side: "right",
      content: <ExplorerPanel standalone hasDependency={false} />,
    },
    {
      id: "withDependencies",
      title: "With dependencies",
      description: "Project and read-only dependency roots are shown together.",
      side: "left",
      content: <ExplorerPanel standalone hasDependency />,
    },
    {
      id: "constrained",
      title: "Long paths and deep nesting",
      description: "Full paths remain available when the tree exceeds the panel width.",
      side: "left",
      content: <ExplorerPanel standalone hasDependency constrained />,
    },
  ];
  return (
    <>
      <PageHeader
        title="Explorer"
        description="Project files and registered read-only dependency roots within the current project boundary."
      />
      <Section id="explorer-panel" title="Project explorer">
        <div className="workspace-state-grid workspace-explorer-state-grid">
          {states.map((state) => (
            <WorkspaceGuideState
              key={state.id}
              title={state.title}
              description={state.description}
              guide={explorerGuides[state.id]}
              side={state.side}
