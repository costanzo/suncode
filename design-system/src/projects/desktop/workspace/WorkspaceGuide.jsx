...
export function WorkspaceGuideTitle({ title, open, onClick }) {
  return (
    <button type="button" className="workspace-guide-title" aria-expanded={open} onClick={onClick}>
      <h3>{title}</h3>
...
export function WorkspaceGuideState({
  title,
  description,
  guide,
  side = "right",
  ...
  return (
    <div className={`workspace-guide-state ${open ? "is-guide-open" : ""} ${className}`}>
      <WorkspaceGuideTitle title={title} open={open} onClick={onToggle} />
      <p>{description}</p>
      {open && (
        <WorkspaceGuide
