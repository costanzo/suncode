...
Final agent-handle close consumes the blocking Rust SDK and waits for explicit core shutdown before destroying its Tokio runtime. Cleanup errors are written to diagnostics because the stable C close function has no return value. Subscription handles must still close before the final agent handle so callback threads are joined before core event streams are retired.
