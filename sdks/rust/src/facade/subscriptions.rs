...
use suncode_agent::{
    AgentAttentionEvent, AgentEvent, AgentEventSubscription, AgentEventSubscriptionControl,
    AttentionEventSubscription, AttentionEventSubscriptionControl, AttentionReceiveError,
    EventReceiveError,
};
...
impl From<AttentionReceiveError> for SubscriptionError {
    fn from(value: AttentionReceiveError) -> Self {
        match value {
            AttentionReceiveError::Lagged { missed } => Self::Lagged { missed },
            AttentionReceiveError::Closed => Self::Closed,
            AttentionReceiveError::Empty => Self::Empty,
        }
    }
}

#[derive(Clone)]
pub struct AttentionEventStreamControl {
    inner: AttentionEventSubscriptionControl,
}

impl AttentionEventStreamControl {
    pub fn close(&self) {
        self.inner.close();
    }
}

pub struct AttentionEventStream {
    inner: AttentionEventSubscription,
    terminated: bool,
}

impl AttentionEventStream {
    pub(super) fn new(inner: AttentionEventSubscription) -> Self {
        Self {
            inner,
            terminated: false,
        }
    ...
    pub fn control(&self) -> AttentionEventStreamControl {
        AttentionEventStreamControl {
            inner: self.inner.control(),
        }
    }

    pub async fn recv(&mut self) -> Result<Arc<AgentAttentionEvent>, SubscriptionError> {
        if self.terminated {
    ...
    pub fn blocking_recv(&mut self) -> Result<Arc<AgentAttentionEvent>, SubscriptionError> {
        if self.terminated {
            return Err(SubscriptionError::Closed);
        }
    ...
    pub fn try_recv(&mut self) -> Result<Arc<AgentAttentionEvent>, SubscriptionError> {
        if self.terminated {
            return Err(SubscriptionError::Closed);
        }
    ...
    fn record_terminal_result(
        &mut self,
        result: Result<Arc<AgentAttentionEvent>, SubscriptionError>,
    ) -> Result<Arc<AgentAttentionEvent>, SubscriptionError> {
        if matches!(
            result,
...
impl Stream for AttentionEventStream {
    type Item = Result<Arc<AgentAttentionEvent>, SubscriptionError>;
...
impl FusedStream for AttentionEventStream {
