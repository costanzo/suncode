...
public sealed class RuntimeSdk : IDisposable
{
    ...
    private RuntimeSdk(IntPtr handle) => _handle = handle;

    public static Task<RuntimeSdk> OpenAsync() => Task.Run(() =>
    {
        lock (SharedHandleLock)
        {
            if (_sharedHandle == IntPtr.Zero)
            {
                var version = NativeMethods.suncode_runtime_sdk_abi_version();
                if (version != AbiVersion)
                {
                    throw new SdkException("abi_mismatch", $"Runtime ABI {version} is not supported; expected {AbiVersion}");
                }

                _sharedHandle = NativeMethods.suncode_runtime_sdk_open_default(out var error);
                if (_sharedHandle == IntPtr.Zero)
                {
                    throw new SdkException("runtime_unavailable", TakeString(error, true) ?? "SunCode runtime could not be started");
                }
            }

            _sharedHandleReferences++;
            return new RuntimeSdk(_sharedHandle);
        }
    });

    public Task<JsonObject> HealthAsync() => CallAsync(NativeMethods.suncode_runtime_sdk_health);
    public Task<JsonObject> DiagnosticsAsync() => CallAsync(NativeMethods.suncode_runtime_sdk_diagnostics);
    public Task<JsonObject> ListModelsAsync() => CallAsync(NativeMethods.suncode_runtime_sdk_list_models);
    public Task<JsonObject> ListCredentialsAsync() => CallAsync(NativeMethods.suncode_runtime_sdk_list_credentials);
    public Task<JsonObject> ListProjectsAsync() => CallAsync(NativeMethods.suncode_runtime_sdk_list_projects);
    ...
    public Task<JsonObject> ListProjectSettingsAsync(string projectId) => WithUtf8Async(
        [projectId], values =>
            NativeMethods.suncode_runtime_sdk_list_settings(_handle, values[0], IntPtr.Zero));
    ...
    public Task<JsonObject> RemoveCredentialAsync(string provider) => WithUtf8Async(
        [provider], values => NativeMethods.suncode_runtime_sdk_remove_credential(_handle, values[0]));

    public Task<JsonObject> OpenProjectAsync(string path) => WithUtf8Async(
        [path], values => NativeMethods.suncode_runtime_sdk_open_project(_handle, values[0], IntPtr.Zero));
    ...
    public Task<JsonObject> GitDiffAsync(string projectId, string scope, string path) => WithUtf8Async(
        [projectId, scope, path], values => NativeMethods.suncode_runtime_sdk_git_diff_file(_handle, values[0], values[1], values[2]));

    public Task<JsonObject> ListSessionsAsync(string projectId) => WithUtf8Async(
        [projectId], values => NativeMethods.suncode_runtime_sdk_list_sessions(_handle, values[0]));

    public Task<JsonObject> CreateSessionAsync(string projectId, string? title, string? model) => WithNullableUtf8Async(
        [projectId, title, model], values => NativeMethods.suncode_runtime_sdk_create_session(_handle, values[0], values[1], values[2]));

    public Task<JsonObject> RenameSessionAsync(string sessionId, string title) => WithUtf8Async(
        [sessionId, title], values => NativeMethods.suncode_runtime_sdk_rename_session(_handle, values[0], values[1]));
    ...
    public Task<JsonObject> RestoreCheckpointAsync(string manifestId, string sessionId) => WithUtf8Async(
        [manifestId, sessionId], values => NativeMethods.suncode_runtime_sdk_restore_checkpoint(_handle, values[0], values[1]));
    ...
    public Task<JsonObject> SubmitTurnAsync(string sessionId, string input, string model) => WithUtf8Async(
        ...
        values => NativeMethods.suncode_runtime_sdk_submit_turn(_handle, values[0], values[1], values[2], values[3]));

    public Task<JsonObject> CancelTurnAsync(string sessionId, string turnId) => WithUtf8Async(
        [sessionId, turnId], values => NativeMethods.suncode_runtime_sdk_cancel_turn(_handle, values[0], values[1]));

    public Task<JsonObject> ResolveApprovalAsync(string approvalId, string decision) => WithUtf8Async(
        [approvalId, decision], values => NativeMethods.suncode_runtime_sdk_resolve_approval(_handle, values[0], values[1]));
    ...
    {
        var json = TakeString(response, true) ?? throw new SdkException("invalid_response", "Runtime returned no response");
        var envelope = JsonNode.Parse(json) as JsonObject
            ?? throw new SdkException("invalid_response", "Runtime returned malformed JSON");
        ...
        throw new SdkException(
            error?["code"]?.GetValue<string>() ?? "runtime_unavailable",
            error?["message"]?.GetValue<string>() ?? "Runtime SDK call failed");
    }
    ...
    private static string? TakeString(IntPtr value, bool free)
    {
        ...
        finally { if (free) NativeMethods.suncode_runtime_sdk_string_free(value); }
    }

    private void ThrowIfDisposed()
    {
        if (_disposed || _handle == IntPtr.Zero) throw new ObjectDisposedException(nameof(RuntimeSdk));
    }
    ...
    public void Dispose()
    {
        ...
        lock (SharedHandleLock)
        {
            if (_handle != IntPtr.Zero && _handle == _sharedHandle)
            {
                ...
                {
                    NativeMethods.suncode_runtime_sdk_close(_sharedHandle);
    ...
    private sealed class Subscription : IDisposable
    {
        ...
        public Subscription(IntPtr runtime, string sessionId, long after, Action<string> onEvent)
        {
            ...
            try
            {
                _subscription = NativeMethods.suncode_runtime_sdk_subscribe_session(
                    runtime, session, after, Callback, GCHandle.ToIntPtr(_callbackHandle), out var error);
