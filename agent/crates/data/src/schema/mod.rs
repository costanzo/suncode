...
pub(crate) fn table_names(connection: &mut SqliteConnection) -> QueryResult<Vec<String>> {
    #[derive(diesel::QueryableByName)]
    struct NameRow {
        #[diesel(sql_type = diesel::sql_types::Text)]
        name: String,
...
pub(crate) fn session_message_excludes_usage_column(
    ...
    let columns = sql_query("PRAGMA table_info(session_message)").load::<ColumnRow>(connection)?;
...
pub(crate) fn session_call_includes_provider_ids(
    ...
    let columns = sql_query("PRAGMA table_info(session_call)").load::<ColumnRow>(connection)?;
    Ok(columns
        .iter()
