...
- `cargo test --manifest-path agent/Cargo.toml -p suncode-db conversation_turns_group_messages_and_tools`: passed.
- `cargo test --manifest-path agent/Cargo.toml -p suncode-agent session_snapshot_serializes_normalized_conversation_turns`: passed.
...
- `cargo test --manifest-path agent/Cargo.toml --workspace`: passed, 86 unit tests and all doc tests.
