...
- `src/components/universal/UniversalComponentsPage.jsx` — universal inventory composition.
...
Universal component markup and interaction state stay co-located with their documentation under `components/universal/<component>/` and are exported through that directory's `index.js`. `UniversalComponentsPage.jsx` imports and composes those modules; it does not duplicate their implementations.

## Data and control flow

The root shell reads `window.location.hash`, resolves a route from a static catalog, derives the active primary module, and renders the owning layer's page component. Primary navigation writes the default child route for a module; first-level sidebar links write routes within that active module; second-level sidebar links scroll to stable section IDs within the selected route. Theme state updates the root `data-theme` attribute and is stored in local storage when available.
