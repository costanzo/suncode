...
Image upload accepts PNG, JPEG, GIF, WebP, BMP, and AVIF file extensions. Original files are bounded to 20 MiB and thumbnail payloads to 1 MiB. `submit_turn_with_attachments` accepts at most three unique IDs, verifies same-session ownership and file availability, and rejects models that do not advertise `capabilities.vision`. Accepted user messages persist `image_ref` content parts; provider requests resolve those references to data URLs only at call time, and provider traces do not retain model request content. Image-bearing submissions are rejected rather than queued behind an active turn so their files cannot be removed before admission. The original text-only ABI remains a compatibility wrapper with an empty image list.
...
Provider exchange lifecycle events are durable: `provider.exchange.started`, `provider.exchange.completed`, and `provider.exchange.failed`. They project into the provider-exchange query surface and may be used by clients to refresh an open trace drawer. `provider.exchange.progress` is a transient, non-persisted event emitted while a built-in provider transfers request or response body bytes. Its payload contains exchange and turn IDs, provider and model IDs, and cumulative `uploaded_bytes` and `downloaded_bytes`. Clients derive rates from successive samples; these are HTTP body rates, not device network-interface rates. Core coalesces samples on a 100ms interval and publishes a final sample when the provider call ends.
