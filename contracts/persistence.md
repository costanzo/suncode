...
SunCode currently has one 17-table schema and no general database migration or version metadata. Initialization applies the ordered schema and data manifests transactionally. Reopening the current schema is idempotent. The built-in-agent delivery adds `subagent_invocation` and the nullable/defaulted session classification columns to the immediately preceding schema; the earlier additive `mcp_server` compatibility path remains accepted. A database with unexpected application tables is rejected without conversion. The current project identity table is singular `project`; a database containing the former `projects` table is therefore incompatible and is not renamed automatically.
