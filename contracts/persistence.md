...
SunCode currently has one 16-table schema and no general database migration or version metadata. Initialization applies the ordered schema and data manifests transactionally. Reopening the current schema is idempotent, and the MCP delivery includes one narrowly additive compatibility path that creates `mcp_server` when opening the immediately preceding 15-table schema. A database with any other missing or unexpected application table is rejected without conversion. The current project identity table is singular `project`; a database containing the former `projects` table is therefore incompatible and is not renamed automatically.
