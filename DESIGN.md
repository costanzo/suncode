...
The visual review surface starts at [`design-system/index.html`](design-system/index.html), the single Vite/React entry for shared foundations, component states, platform boundaries, project mappings, rules, and assets. Its navigation has three levels: Core, Components, Platforms, Projects, and Agents switch the primary content domain from the upper-right; each switch first opens a concise card-based module index without redundant ownership or path labels; and the left sidebar exposes the detailed hierarchy. Every expandable child entry in Core, Components, Platforms, Projects, and Agents is a stable route rather than a scroll target: parent pages remain concise card indexes, while the final child page owns its content. The tree can recurse for large surfaces: Desktop Workspace has one complete project-window composition plus stable child routes for Sessions, Explorer, Conversation, Review, Source control, and Provider trace. The top-level switcher uses names only, while detailed hierarchy remains in the sidebar. All React source lives under [`design-system/src/`](design-system/src/), organized by application, core pages, components, platforms, projects, agents, shared primitives, styles, and assets. Each component folder under [`design-system/src/components/universal/`](design-system/src/components/universal/) owns its specimen and stable export; [`design-system/src/components/universal/modules/`](design-system/src/components/universal/modules/) owns the category pages; and [`design-system/src/components/universal/UniversalComponentsPage.jsx`](design-system/src/components/universal/UniversalComponentsPage.jsx) is only the module index. Token sources live under [`design-system/src/styles/tokens/`](design-system/src/styles/tokens/), reusable product images and icons are cataloged in [`design-system/src/assets/`](design-system/src/assets/), and browser-direct files such as the favicon live under [`design-system/public/assets/`](design-system/public/assets/). Review the corresponding hash route before introducing a new module or component. React/Vite is design-review tooling only, not a production web client or Phase 1 runtime dependency.
...
The Workspace footer's right side always shows compact aggregate provider transfer rates while a project is open, using upward and downward arrows followed by a rate such as `↑ 12.4 KiB/s   ↓ 84.2 KiB/s`; with no observed traffic it remains visible as `↑ 0 B/s   ↓ 0 B/s`. Rates use HTTP request and response body bytes observed by Rust provider adapters, binary units, and a one-second averaging window refreshed every second; they do not represent whole-device network throughput. The indicator is one compact monospace line within the existing 20px footer. Its tooltip identifies active providers and models and shows cumulative body bytes. The footer also keeps the running-session count visible while a project is open, including `0 running` when idle; idle uses a single static dot, while active sessions restore the running pulse indicator.
...
The centered Workspace title is a current-content switcher rather than a fixed session label. It shows the active primary-session title while Conversation is visible, the active file name while the read-only Editor is visible, and the delegated title while a child session is being inspected, with a type icon and trailing down chevron. Activating the title opens an anchored `Recently viewed` menu that interleaves primary sessions, child sessions, and files in most-recent-first order, deduplicates them by stable identity, and retains at most 20 entries internally. The current item remains in that internal history but is omitted from the open menu, so the list contains only other content and the previously active item appears after a switch. Primary-session rows show session recency; child-session rows show the built-in agent identity and terminal or active state; file rows show the project-relative or dependency display path. A restrained blue-violet icon surface and type label identify primary sessions, a quiet steel agent surface identifies child sessions, and a restrained teal-green pair identifies files; row text remains neutral so color is a redundant cue rather than the only distinction. The menu count and empty state reflect only its visible, non-current entries. Selecting a row changes the central content immediately, moves that item to the front, and closes the menu without changing project identity or making file or child-session content editable. The switcher supports Escape/outside-click dismissal, keyboard focus, long-title/path elision, a bounded scrolling list, an empty state, and constrained-width geometry.

The Workspace right bay has two mutually exclusive destinations: `Review` and `Child sessions`. Child sessions lists only the delegated sessions belonging to the selected primary session and provides no create, rename, pin, archive, retry, or direct-message actions. Each row shows the delegated title, built-in agent display name, activity state, and recency. Selecting a child session replaces the central Conversation or Editor with a read-only child-session detail while preserving the primary session as the authority and conversation root. The detail has an agent identity header, model/duration/tool metadata, and a chronological task, tool, approval, failure, and result timeline; it never renders a composer. A pending machine-operation approval appears inline in this detail with `Allow once` and `Deny`; approval is an authority decision, not direct chat with the child agent. At constrained widths the right bay yields before the central detail becomes unusable, while ContentSwitcher remains an access path to recently inspected child sessions.

Workspace presentation state resumes per project when its window is reopened. The left region stores one extensible destination (`closed`, Sessions, or Explorer), the right region stores one extensible destination (`closed`, Review, or Child sessions), and the mutually exclusive bottom drawer stores `closed`, Source control, Provider trace, or Tool activity. Normal window bounds, maximized/full-screen state, navigation width, review width, bottom-drawer height, current central file/primary-session/child-session, last selected primary session, and the bounded recent-content history are restored. Window bounds are constrained to an available display before presentation, and responsive breakpoints may temporarily hide a restored region without replacing the user's stored destination. Settings similarly reopens to its last global first-level or provider destination. This restoration changes no agent, project, session, file, or authority data and adds no visual state.
...
- **New-session focus:** After a session is successfully created, dismiss its naming dialog and move keyboard focus to the compact composer when text entry is available. Renaming, cancellation, and failed creation preserve their existing focus behavior.
...
- **Conversation scrolling:** Entering a session positions the conversation at its newest message once. Submitting a new turn re-enters follow mode so the sent user message and its live output remain visible; completion of the active turn may move the viewport to the newest message after the final response is committed. If the user scrolls away during that turn, later assistant, tool, and layout updates preserve the current viewport. When the viewport is away from the newest message, show a compact circular down-arrow control above the composer; activating it returns to the latest message, and the control remains hidden while already at the bottom.
...
- **Catalog:** Selecting the Agents parent opens an overview list on the right, matching the Model providers overview pattern. Each row shows the display name, a concise description, and the unique machine name. Selecting an overview row or a navigation child opens that agent's read-only detail with the stable ID, definition version, model policy, tool-call limit, exact built-in tool allowlist, MCP policy, delegation policy, and direct-user-chat policy.
- **Detail rhythm:** The identity header, metadata grid, allowed-tool list, and boundaries remain distinct reading groups. Use 24–30px separation between groups, 12px internal metadata padding, and 42px boundary rows so section labels never touch the preceding border or tool chips.
- **Mutability:** The surface exposes no create, edit, enable, disable, reorder, or delete action. The compact `READ ONLY` badge communicates the catalog's immutable scope without adding a separate explanatory notice.
- **Authority:** Tool availability is described as an allowlist, never as an authority grant. The detail communicates scope through the `Allowed tools` and `Boundaries` sections without adding a separate authority notice.
- **Responsive behavior:** The nested navigation follows the same collapse, focus, selection, and narrow-window behavior as Model providers. The detail remains a single reading column, and identifiers and tool names stay selectable, monospace, and overflow-safe.
...
Merged Workspace windows place a compact project tab strip below the Workspace title bar and above its main content. The active project tab uses the standard surface and accent underline; inactive tabs remain neutral, scroll horizontally instead of shrinking below a usable label width, and retain a visible keyboard focus ring. A tab's close action appears on hover or keyboard focus at its trailing edge and closes that project window; when one project remains, it returns to a standalone window, and closing the last project returns to ProjectHub. The native `Window` menu exposes `Merge All Windows`, disabled when fewer than two project windows are open. Dragging a tab by 28 screen pixels or more detaches it into a standalone Workspace window without resetting project state; when only one project remains, it returns to a standalone window as well.
